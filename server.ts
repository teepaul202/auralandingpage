...
// Initialize SQLite database with in-memory fallback
let db: any = null;
const inMemorySubscriptions: Set<string> = new Set();
...
// Helper to send email alerts on new contact form submissions using Resend API
async function sendContactEmail(name: string, email: string, inquiryType: string, message: string) {
      ...
      body: JSON.stringify({
        ...
        html: `
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 10px; background-color: #fbfbf6; color: #1e251a;">
            ...
            <p style="font-size: 10px; color: #888; text-align: center; margin: 0;">© ${new Date().getFullYear()} Aura Global Employment Schema Network.</p>
          </div>
        `
      })
    });
...
app.post('/api/contact', (req, res) => {
  ...
  if (db) {
    ...
    // In-memory fallback
    inMemoryMessages.push({ name, email, message, inquiryType: resolvedInquiryType, created_at: new Date() });
...
// API Endpoint: Fetch Socials and Live GitHub Stats
app.get('/api/socials', async (req, res) => {
  ...
  try {
    ...
    if (repoRes.ok) {
      ...
      responseData.github.recentRepos = repoData.map((repo: any) => ({
        name: repo.name,
    ...
    // Provide some mock repositories as fallback so it looks complete
...
if (fs.existsSync(distPath)) {
  ...
  // Simple welcome endpoint for dev modes when dist doesn't exist
  app.get('/', (req, res) => {
